...
Everything else works without it, including the XMI and JSON serializers, the JSON
instantiator, the metamodel builder, the transformer, the template language and the
metamodel generator.
...
This puts 'lib' on the load path, so the tests exercise the working tree. Running
test/rgen_test.rb directly without '-I lib' loads an installed rgen gem instead.


== Documentation

RDoc documentation is available at Github: https://mthiede.github.io/rgen/
