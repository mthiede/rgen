...
class XMIInstantiator < AbstractXMLInstantiator
  ...
  def initialize(env, fix_map=nil, loglevel=INFO)
    @env = env
    @fix_map = fix_map || {}
    @loglevel = loglevel
    @rolestack = []
  ...
  def tag_map(tag, attributes)
    tmap = @fix_map[:tag_map]
    proc = tmap[tag] if tmap
    if proc
      puts "fixing tag #{tag}"
      proc.call(tag, attributes)
    end 
  end
        
  def feature_name_map
    @fix_map[:feature_names] || {}
  end
  
  def map_value(attr_name, value)
  ...
  def start_tag(prefix, tag, namespaces, attributes)
    ...
    if tag =~ /\w+\.(\w+)/
      role_name = feature_name_map[$1] || $1
      ...
      @resolver_descs << ResolverDescription.new
      @resolver_descs.last.object = @elementstack.last
      @resolver_descs.last.attribute = @rolestack.last.name
      @resolver_descs.last.value = attributes["xmi.idref"]
      @resolver_descs.last.many = @rolestack.last.many      
      @elementstack.push nil
    else
      obj = tag_map(tag, attributes) 
      unless obj
        unless mod
          log WARN, "Ignoring tag #{tag}"
          return
        end
        obj = mod.const_get(tag).new
      end
      @env << obj
      eRef = @rolestack.last
      if eRef && eRef.many
  ...
  def set_attribute(attr, value)
    ...
    if attr == "xmi.id"
      ...
      attr_name = feature_name_map[attr] || attr
      ...
      if eFeat.is_a?(RGen::ECore::EReference)
        if map_value(attr_name, value).is_a?(eFeat.eType.instanceClass)
          @elementstack.last.setGeneric(attr_name, map_value(attr_name, value))
        else
          @resolver_descs << ResolverDescription.new
          @resolver_descs.last.object = @elementstack.last
          @resolver_descs.last.attribute = attr_name
          @resolver_descs.last.value = value
          @resolver_descs.last.many = eFeat.many
        end
      else
        value = map_value(attr_name, value) || value
